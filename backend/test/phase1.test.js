...
test('file and folder names are safely escaped in React frontend, never injected via dangerouslySetInnerHTML', async () => {
  const driveSource = await readFile(new URL('../../frontend/src/features/drive/DrivePage.tsx', import.meta.url), 'utf8');
  assert.match(driveSource, /\{folder\.name\}/);
  assert.match(driveSource, /\{name\}/);
  assert.doesNotMatch(driveSource, /dangerouslySetInnerHTML/);
});
